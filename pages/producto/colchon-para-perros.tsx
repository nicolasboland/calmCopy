...
import landingContent from '@/jsons/ProductContent/colchonPerro.json'
import productLandingRequests from '@/utils/productLandingRequests';

export const ColchonPerro = ({landingSEO, graphImageObject, graphWebPage, relatedProducts, defaultProducts, product}: IProductLanding) => {
  useEffect(() => {
    topPage()
  }, [])
...
export const getStaticProps: GetStaticProps<IProductLanding> = async () => {
 ...
 const graphWebPage = {
  ...
  const responses = await productLandingRequests(landingContent["productId"], landingContent["key"], "colchones_product_seo_data")
